...
<nav class="navbar navbar-expand-lg bg-white pl-4 text-danger justify-content-between" style="border-bottom: 1px solid #c8ced3">
  <h2>{{ trans('acl::global.edit') }} {{ trans('acl::cruds.permission.title_singular') }}</h2>
...
<div class="card">
  <div class="card-body">
    <form method="POST" action="{{ route(config('acl.route.as') . "permissions.update", [$permission->id]) }}" enctype="multipart/form-data">
      @method('PUT')
      @csrf
